...
{
//    return exp(-t*t);
    return 8.0*(15.0+15.0*t+6.0*t*t+t*t*t)*exp(-t);
}
...
__kernel void kern(float tf, float dtf, int N, int m, int gr,
    __global double * cq, __global double * calpha, __global double * grid, float s)
{
    double t;
    if(s>0.0f) t=(double)tf;
    else t=1.0-((double)tf);
    double dt=(double)(s*dtf);
...
G={g=function (x) return 8*(15+15*x+6*x*x+x*x*x)*math.exp(-x) end,
dg=function (x) return -8* math.exp(-x)* x*(3 + 3*x + x*x) end,
delta=0.00 }
